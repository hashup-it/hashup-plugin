...
export const GameProvider = ({ children, cartridgeAddress }: { children: ReactNode, cartridgeAddress: string }) => {
	...
	const {
		data: queryData,
		isLoading
	} = useMoralisQuery('Games', (query) => query.equalTo('address', cartridgeAddress?.toLowerCase()));
	...
	useEffect(() => {
		if (isLoading) {
			...
			setProvidedGameData({ ...payload, price: ethers.utils.formatUnits(payload.price, 6), address: cartridgeAddress });
