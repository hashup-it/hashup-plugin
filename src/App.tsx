...
import { VStack, theme, Box, Heading } from '@chakra-ui/react';
...
const options = {
  [PLUGIN_MODE.DEVELOPMENT]: ({ gameBalance }: any) => (
    <>
      <VStack position="fixed" zIndex="3" top="0" right="0" spacing="0" w="476px" fontSize="0.625rem" color={theme.colors.gray[400]}>
        <Main />
      </VStack>
      {+gameBalance < 1 && (
        <Box position="fixed" zIndex="2" bg="rgba(0,0,0,0.9)" top="0" left="0" right="0" bottom="0" textAlign="left">
          <Heading position="absolute" bottom="1rem" left="1rem" >Not allowed,<br />buy at least one game</Heading>
        </Box>
      )}
    </>
  ),
