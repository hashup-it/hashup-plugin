    ...
    return (
        <chakra.div __css={containerStyles}>
            <Tooltip label='Copied' isOpen={hasCopied}>
                <Button size="h-md" gridArea="referral" flexDir="column" textTransform="uppercase" onClick={copyReflink}>
                    Copy referral<br />
                    <Text variant="secondary" textTransform="none" color="black">Earn <Text as="span" color="white">2%</Text> on every transaction</Text>
                </Button>
            </Tooltip>
            <Button size="h-md" as={Link} href="https://gamexplorer.io" isExternal gridArea="gamexplorer" variant="outline" textTransform="uppercase">Gamexplorer</Button>
            <Button size="h-md" onClick={addTokenToMetamask} gridArea="addToMetamask" leftIcon={<MetamaskIcon />} textTransform="uppercase" variant="ghost">
